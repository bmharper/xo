...
DumbControl::DumbControl(xo::DomNode* root) {
	BoundThread = std::this_thread::get_id();
	if (root)
		Bind(root);
}
...
DumbControl::~DumbControl() {
}

void DumbControl::ObservableTouched(Observable* target) {
	SetDirty();
}

void DumbControl::Bind(xo::DomNode* root) {
	Root = root;
...
void DumbControl::OnDocLifecycle(Event& ev) {
	if (ev.DocLifecycle != DocLifecycleEvents::DispatchEnd)
		return;
	DumbControl* self = (DumbControl*) ev.Context;
	if (self->Dirty) {
		self->Render();
		self->Dirty = false;
	}
