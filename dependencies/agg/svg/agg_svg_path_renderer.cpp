...
//        throw exception("curve3(x, y) : NOT IMPLEMENTED YET");
    ...
    //------------------------------------------------------------------------
    void path_renderer::arc(double rx, double ry, double angle,        // A, a
                            double large_arc_flag, double sweep_flag,
                            double x, double y, bool rel)
    {
		if(rel)
		{
			m_storage.arc_rel(rx, ry, angle, large_arc_flag != 0, sweep_flag != 0, x, y);
		} else
		{
			m_storage.arc_to(rx, ry, angle, large_arc_flag != 0, sweep_flag != 0, x, y);
		}
	}
    ...
    {
		// capture the first vertex of the path, and save it as the current position,
		// because the SVG current pen position is retained when a subpath is closed.
		//double x;
		//double y;
		//m_storage.first_polygon_vertex(&x, &y);
