...
namespace svg
{
    ...
    const char path_tokenizer::s_numeric_nodot[] = "Ee0123456789";
        ...
        : m_path(0), m_last_command(0), m_last_number(0.0)
    {
        ...
        init_char_mask(m_numeric_nodot_mask, s_numeric_nodot);
    ...
    bool path_tokenizer::parse_number()
    {
        ...
        {
            *buf_ptr++ = *m_path++;
			// Only allow one dot, because it's legal to write .12.34 to specify a pair of numbers (.12 and .34)
			if (m_path[-1] == '.')
				break;
        }

		// Copy numeric characters after the dot
        while(buf_ptr < buf+255 && is_numeric_nodot(*m_path))
        {
            *buf_ptr++ = *m_path++;
        }
        *buf_ptr = 0;
