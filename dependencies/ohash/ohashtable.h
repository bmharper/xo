...
public:
	...
	**/
	...
	table& operator=(table&& b) {
		...
		mSize          = b.mSize;
		mMask          = b.mMask;
		mProbeOffset   = b.mProbeOffset;
		mCount         = b.mCount;
		mAge           = b.mAge;
		mFillRatio     = b.mFillRatio;
		mFillRatioMul4 = b.mFillRatioMul4;
	...
	**/
	void resize(hashsize_t newsize) {
		...
		if (newsize < 2)
			...
			newsize = next_power_of_2(newsize);
		...
		for (hashsize_t i = 0; i < osize; i++) {
			...
			if (os == SFull)
				insert_no_check(std::move(odata[i]));
		}
		if (odata != NULL)
			delete[] odata;
		if (ostate != NULL)
...
protected:
	...
	hashsize_t insert_no_check(TData&& obj) {
		insert_check_resize();
		autoshrink();
	...
	**/
		...
		for (hashsize_t i = 0; i != mSize; i++) {
			...
			if (ks == SFull) {
				if (TGetKeyFunc::equals(TGetKeyFunc::getkey(mData[pos]), TGetKeyFunc::getkey(obj))) {
					// key already present
					if (overwrite) {
						TGetValFunc::getval_mutable(mData[pos]) = TGetValFunc::getval(obj);
						return pos;
					} else
						return npos;
				}
	...
	int _find(const TKey& key) const {
		...
		KeyState ks = get_state(mState, pos);
		if (ks == SFull && TGetKeyFunc::equals(TGetKeyFunc::getkey(mData[pos]), key))
			return pos;
		else if (ks == SNull)
			return npos;
		else {
			...
			while (get_state(mState, pos) != SNull) {
				if (get_state(mState, pos) == SFull && TGetKeyFunc::equals(TGetKeyFunc::getkey(mData[pos]), key))
					return pos;
				pos = table_pos(hkey, ++i);
				if (pos == first)
					break;
	...
	void autoshrink() {
		if (mAutoShrink && mAge * 2 > mCount)
			resize((hashsize_t)((double) (mCount + 1) * mFillRatio * 1.5));
	}
