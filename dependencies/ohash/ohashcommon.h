...
inline hashsize_t table_pos(hashsize_t mask, hashsize_t probeOffset, hashkey_t key, uint32_t i)
{
	key = fold(key);
	uint32_t mul = key >> 1;
...
template<typename T> hashkey_t gethashcode(const T& k)			{ return (hashkey_t) k; }
...
template<> inline hashkey_t gethashcode(void* const& k)			{ return hashpointer(k); }

template<> inline hashkey_t gethashcode(const bool& k)			{ return (hashkey_t) k; }
template<> inline hashkey_t gethashcode(const char& k)			{ return (hashkey_t) k; }
template<> inline hashkey_t gethashcode(const wchar_t& k)		{ return (hashkey_t) k; }

template<> inline hashkey_t gethashcode(const int8_t& k)		{ return (hashkey_t) k; }
template<> inline hashkey_t gethashcode(const int16_t& k)		{ return (hashkey_t) k; }
template<> inline hashkey_t gethashcode(const int32_t& k)		{ return (hashkey_t) k; }
template<> inline hashkey_t gethashcode(const int64_t& k)		{ return (hashkey_t) ((uint32_t) k ^ (uint32_t) ((uint64_t) k >> 32)); }
template<> inline hashkey_t gethashcode(const uint8_t& k)		{ return (hashkey_t) k; }
template<> inline hashkey_t gethashcode(const uint16_t& k)		{ return (hashkey_t) k; }
template<> inline hashkey_t gethashcode(const uint32_t& k)		{ return (hashkey_t) k; }
template<> inline hashkey_t gethashcode(const uint64_t& k)		{ return (hashkey_t) ((uint32_t) k ^ (uint32_t) (k >> 32)); }
...
template<> inline hashkey_t gethashcode(const float& k)			{ float copy = k; return *((int32_t*) &copy); }
template<> inline hashkey_t gethashcode(const double& k)		{ double copy = k; int32_t* p = (int32_t*) &copy; return p[0] ^ p[1]; }

template<> inline hashkey_t gethashcode(const char* const& k)	{ return stringhash_sdbm_nullterm(k); }
template<> inline hashkey_t gethashcode(const std::string& k)	{ return stringhash_sdbm(k.c_str(), k.length()); }
template<> inline hashkey_t gethashcode(const std::wstring& k)	{ return stringhash_sdbm(k.c_str(), k.length()); }
